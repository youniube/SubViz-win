...
function zhErr(s){s=String(s||'');var raw=s;var lower=s.toLowerCase();if(!s)return '';if(s.indexOf('不支持该协议')>=0||lower.indexOf('unsupported protocol')>=0)return '不支持该协议进行落地检测';if(s.indexOf('落地查询失败')>=0||lower.indexOf('landing lookup failed')>=0)return '落地查询失败：所有备用接口或临时代理尝试均失败';if(lower.indexOf('timeout')>=0||s.indexOf('超时')>=0)return '请求超时：节点不可用、速度过慢，或查询接口被阻断';if(lower.indexOf('websocket closed')>=0)return 'WebSocket 被服务端关闭：通常是 Host/SNI/path 不匹配，或 CDN/服务端拒绝握手';if(lower.indexOf('ss missing cipher')>=0||s.indexOf('SS 节点缺少')>=0)return 'SS 节点缺少加密方式或密码：多半是解析没有识别 cipher/password';if(lower.indexOf('mihomo proxy descriptor')>=0)return 'mihomo 临时代理策略创建失败：该节点参数可能不兼容';if(lower.indexOf('http 403')>=0)return '查询接口返回 403：接口拒绝访问，或该节点出口被限制';var m=s.match(/HTTP\s*(\d+)/i);if(m)return '查询接口返回 HTTP '+m[1];if(s.indexOf('节点数据不是有效 JSON')>=0||lower.indexOf('invalid node json')>=0)return '节点数据格式异常';if(s.indexOf('内部 GEOIP')>=0||lower.indexOf('internal geoip')>=0)return '内部 GEOIP 查询失败：独立版内部 GEOIP 不可用或查询失败';if(s.indexOf('查询接口返回内容解析失败')>=0||lower.indexOf('parse failed')>=0)return '查询接口返回内容解析失败';if(lower==='failed')return '检测失败';return raw}
...
function gistEnsurePanel(){
  ...
  function bindGistPanel(){
    ...
    var save=gistById('gistSaveToken');if(save&&!save._svGistBound){save._svGistBound=1;save.onclick=function(){var token=gistVal('gistToken');if(!token){st('请先粘贴新的 GitHub Token');return}st('正在保存 GitHub Token 到本地…');gistPost('/api/gist-token/save',{token:token}).then(function(r){if(!r.ok)throw new Error(r.error||'保存失败');gistById('gistToken').value='';gistSetTokenStatus(true,'已保存 / 可访问 Gist API','saved');st('Token 已保存/更新到本地。以后上传时可留空 Token。')}).catch(function(e){gistSetTokenStatus(false,'Token 保存失败','error');st('保存 Token 失败：'+(e.message||e))})}}
    var clear=gistById('gistClearToken');if(clear&&!clear._svGistBound){clear._svGistBound=1;clear.onclick=function(){st('正在清除已保存 Token…');gistPost('/api/gist-token/delete',{}).then(function(r){if(!r.ok)throw new Error(r.error||'清除失败');gistSetTokenStatus(false,'未配置','empty');st('已清除本地保存的 GitHub Token')}).catch(function(e){gistSetTokenStatus(false,'清除 Token 失败','error');st('清除 Token 失败：'+(e.message||e))})}}
