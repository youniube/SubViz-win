...
class MihomoManager {
  ...
  async apiFetch(method, apiPath, bodyObj, timeout = 5000) {
    ...
    const r = await fetchText(requestUrl, { method, headers, body, timeout });
    let obj = null;
  ...
  findLoadedProxyName(name, node, inventory) {
    const inv = inventory || this.proxyInventoryCache.data || { nameSet: new Set(), nodeNameSet: new Set() };
    ...
    const foundAny = foundNode || candidates.find(v => allSet.has(v));
    return {
      found: !!foundNode,
      foundAny: !!foundAny,
      name: foundNode || foundAny || candidates[0] || '',
      candidates,
  ...
  async injectNodes(nodes) {
    ...
    try {
      ...
      logMihomoDebug('订阅已加载到 Mihomo：解析节点 ' + nodes.length + '，写入配置 ' + this.lastBuiltProxyCount + '，API 节点 ' + (diagnostics.mihomoNodeCount || 0) + '，可匹配测速 ' + (diagnostics.testableCount || 0) + '，未加载/未匹配 ' + (diagnostics.missingCount || 0), null, true);
      if (diagnostics.missingCount) logMihomoDebug('未加载/未匹配节点样例', diagnostics.missingSample, true);
