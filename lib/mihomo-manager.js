...
class MihomoManager {
  ...
  async injectNodes(nodes) {
    ...
    try {
      ...
      logMihomoDebug('订阅已加载到 Mihomo：解析节点 ' + nodes.length + '，写入配置 ' + this.lastBuiltProxyCount + '，API 节点 ' + (diagnostics.mihomoNodeCount || 0) + '，可匹配测活 ' + (diagnostics.testableCount || 0) + '，未加载/未匹配 ' + (diagnostics.missingCount || 0));
